...
public:
   ...
   {
      lastMove = currentMove; nMoves++;
   }
   virtual void setPosition(int c, int r) {
      position.set(c, r);
   }
...
{
public:
   PieceDerived(const Position& pos, bool isWhite)
   : Piece(pos, isWhite)
   {
      nMoves = 0;
      fWhite = isWhite;
   ...
   PieceDerived(int c, int r, bool isWhite)
   : Piece(Position(c, r), isWhite)
   {
      nMoves = 0;
      fWhite = isWhite;
